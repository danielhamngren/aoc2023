...
func checkCards(cards string) int {
...
func checkWinningHand(x, y Hand) int {
	// 1. Get the hand type
	handOrderX := checkCards(x.cards)
	handOrderY := checkCards(y.cards)

	if handOrderX == handOrderY {
		// 3. If same, handle draw
		return firstWins(x, y)
	}
...
func day7part1(input []string) int {
	...
	slices.SortFunc(hands, func(i, j Hand) int { return checkWinningHand(i, j) })
	fmt.Println(hands)
...
func Day7() {
	...
	resultPart2 := day7part2(inputDay7(true))
